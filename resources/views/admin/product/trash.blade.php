...
@section('content-header')
    ...
    <div class="box-body table-responsive no-padding">
        <table class="table table-hover">
            ...
            @endif
            <a href="{{ route('product.index') }}" class="btn btn-success" style="margin-left:2%; margin-top:2%"><- Quay về
                    </a>
                    <tbody>
                        <tr>
                            <th>STT</th>
                        ...
                        @forelse ($products as $item)
                            <tr>
                                ...
                                <td>
                                    <img src="{{ asset('images') }}/{{ $item->image }}" alt="" width="150px"
                                        height="150px">
                                </td>
                                <td>{{ $item->created_at }}</td>

                                <td>
                                    <a href="{{ route('product.restore', $item->id) }}" class="btn btn-success">Khôi
                                        phục</a>
                                    <a href="{{ route('product.forceDelete', $item->id) }}"
                                        onclick="return confirm('Xóa vĩnh viễn !')" class="btn btn-danger">Xóa vĩnh viễn</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
