                            ...
                            <form role="form" method="POST" action="{{ route('product.update', $product) }}"
                                ...
                                <div class="box-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group @error('photo') has-error @enderror">
                                                ...
                                                <div class="input-group">
                                                    <!-- Ẩn input type file -->
                                                    <input type="file" id="single_photo_input" name="photo"
                                                        class="form-control" style="display: none;"
                                                        onchange="displaySingleImage(event)">
                                                    <!-- Phần tử label để mở tệp hình ảnh -->
                                                    <label for="single_photo_input"
                                                        class="input-group-addon btn btn-primary">Chọn ảnh</label>
                                                    <!-- Input field để hiển thị tên file ảnh -->
                                                    <input type="text" class="form-control" id="single_image_name"
                                                        value="{{ $product->image }}" readonly>
                                                </div>
                                                <div id="single_image_preview" style="margin-top: 10px;"></div>
                                                <button type="button" class="btn btn-danger" id="remove_image_button"
                                                    style="margin-top: 10px; display: none;"
                                                    onclick="removeSingleImage()">Xóa ảnh</button>
                                                @error('photo')
                                                    <span class="help-block">{{ $message }}</span>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="form-group @error('photos') has-error @enderror">
                                                ...
                                                <div class="input-group">
                                                    <!-- Ẩn input type file -->
                                                    <input type="file" id="multiple_photos_input" name="photos[]"
                                                        class="form-control" style="display: none;"
                                                        onchange="displayMultipleImages(event)" multiple>
                                                    <!-- Phần tử label để mở tệp hình ảnh -->
                                                    <label for="multiple_photos_input"
                                                        class="input-group-addon btn btn-primary">Chọn ảnh</label>
                                                    <!-- Input field để hiển thị tên file ảnh -->
                                                    <input type="text" class="form-control" id="multiple_images_name"
                                                        value="" readonly>
                                                </div>
                                    ...
                                    <div class="form-group @error('description') has-error @enderror">
                                        ...
                                        <textarea name="description" id="editor1" rows="10" cols="20">{{ $product->description }}</textarea>
                                        @error('description')
                                            <span class="help-block">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="parent-container">
                                    ...
                                    <div class="div2">
                                        <img id="single_product_image" src="{{ asset('images/' . $product->image) }}"
                                            alt="{{ $product->name }}"
                                            style="width:240px; border-radius: 5px;background-color: transparent;">
                                    </div>
                                </div>
                                    ...
                                    <div class="col-lg-6">
                                        @if ($i < $imageCount)
                                            <div class="image-wrapper">
                                                <form action="{{ route('deleteimagepro', $product->images[$i]->id) }}"
                                                    method="post" class="image-delete-form">
                                                    @csrf
                                                    @method('delete')
                                                ...
                                                <img src="{{ asset('images_many/' . $product->images[$i]->image) }}"
                                                    class="img-responsive"
                                                    style="height: 90px;width: 130px; border-radius: 5px; margin-bottom: 10px;"
                                                    alt="">
                                            </div>
                                        @endif
                                        @php
                                        ...
                                        @if ($nextIndex < $imageCount)
                                            <div class="image-wrapper">
                                                <form
                                                    action="{{ route('deleteimagepro', $product->images[$nextIndex]->id) }}"
                                                    method="post" class="image-delete-form">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit" class="delete-button1">
                                                ...
                                                <img src="{{ asset('images_many/' . $product->images[$nextIndex]->image) }}"
                                                    class="img-responsive"
                                                    style="height: 100px;width: 120px; border-radius: 5px; margin-bottom: 10px;"
                                                    alt="">
                                            </div>
                                        @endif
                                    </div>
